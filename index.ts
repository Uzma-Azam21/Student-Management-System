...
class Student {
  ...
  view_balance() {
    console.log(
      chalk.bold(
        `Account balance for ${chalk.cyan.bold(this.name)}: $${chalk.cyan.bold(
          this.balance
        )}`
      )
    );
  ...
  pay_fees(amount: number) {
    ...
    console.log(
      chalk.bold(
        `Payment received! ${chalk.cyan.bold(
          this.name
        )}'s fee of $${chalk.cyan.bold(amount)} is now settled.`
      )
    );
    console.log(
      chalk.bold(`Remaining Balance: $${chalk.blue.bold(this.balance)}`)
    );
  }
...
class Student_manager {
      ...
      console.log(
        chalk.bold(
          `Enrollment complete! ${chalk.cyan.bold(
            student.name
          )} is now in ${chalk.cyan.bold(course)}.`
        )
      );
    }
  }
  ...
  view_student_balance(student_id: number) {
    ...
    if (student) {
      ...
      console.log(
        chalk.red.bold("Error: Student ID not recognized. Please re-enter.")
      );
    }
  }
        ...
        let course_input = await inquirer.prompt([
          {
            ...
            message: chalk.rgb(
              ...
              36,
              116
            )("Provide the unique student's ID:"),
          },
          {
            name: "course",
            type: "input",
